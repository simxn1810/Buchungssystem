...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isInteger(wochentag) || wochentag < 0 || wochentag > 6) {
    return NextResponse.json({ fehler: "Ungültiger Wochentag." }, { status: 400 });
  }
  if (!/^\d{2}:\d{2}$/.test(zeitVon) || !/^\d{2}:\d{2}$/.test(zeitBis)) {
    return NextResponse.json({ fehler: "Ungültige Uhrzeit." }, { status: 400 });
  }
  ...
  if (!platz || !platz.aktiv) {
    return NextResponse.json({ fehler: "Platz nicht verfügbar." }, { status: 400 });
  }
